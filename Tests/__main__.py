...
from .. import image_estimates, get_sensor_bands
...
def _get_sensors(model_dir='Weights'):
...
def test_image_estimates():
    ...
    def test_sensor(sensor):
        ...
        est, slices = image_estimates(random_data, sensor=sensor, silent=True)
        chlor_a = est[..., slices['chl']]
        assert (chlor_a.shape == (3, 3, 1)), f'Unexpected shape found: {chlor_a.shape}'
...
def test_benchmarks():
    ...
    for product, benchmarks in product_benchmarks.items():
        ...
        longest = max(map(len, benchmarks)) if len(benchmarks) else 0
...
if __name__ == '__main__':
    ...
    for function in test_functions:
        ...
        try:
            ...
            print(f'\tFailed to run {function.__name__}: {e}')
