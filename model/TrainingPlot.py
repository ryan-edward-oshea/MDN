...
from ..metrics import rmsle, leqznan, mdsa, performance
...
class TrainingPlot:
    ...
    def setup(self):
        ...
        conts = [
            GridSubplot(1, 2 if i in [0, n_row - 1, n_row - 2] else n_col, subplot_spec=o, wspace=0.3 if i else 0.45)
            for i, o in meta]
        ...
        if self.args.animate:
            ...
            ani_writer.setup(fig, ani_path.joinpath('MDN.mp4').as_posix(), dpi=100,
                             frame_prefix=ani_tmp.joinpath('_').as_posix(), clear_temp=False)

    @ignore_warnings
    def update(self, plot_metrics=[mdsa, rmsle]):
        model = self.model
        if hasattr(model, 'session'):
            (prior, mu, sigma), est, avg = model.session.run([model.coefs, model.most_likely, model.avg_estimate],
                                                             feed_dict={model.x: self.train_test})
            train_loss = model.session.run(model.neg_log_pr, feed_dict={model.x: self.data['train']['x_t'][self._idx],
                                                                        model.y: self.data['train']['y_t'][self._idx]})
            test_loss = model.session.run(model.neg_log_pr, feed_dict={model.x: self.data['test']['x_t'],
                                                                       model.y: self.data['test']['y_t']})
        else:
            ...
            train_loss = model.loss(self.data['train']['y_t'][self._idx],
                                    model(self.data['train']['x_t'][self._idx])).numpy()
        ...
        for metric in plot_metrics:
            self.train_losses[metric.__name__].append(
                [metric(y1, y2) for y1, y2 in zip(self.data['train']['y'][self._idx].T, train_est.T)])
            self.test_losses[metric.__name__].append(
                [metric(y1, y2) for y1, y2 in zip(self.data['test']['y'].T, test_est.T)])
        ...
        if model.verbose:
            messages = zip(
                [performance(lbl, y1, y2) for lbl, y1, y2 in zip(self.labels, self.data['test']['y'].T, test_est.T)],
        ...
        for yidx, lbl in enumerate(self.labels):
            ...
            ax.scatter(self.data['test']['y'][:, yidx], test_est[:, yidx], 10, c=test_mixes / prior.shape[1],
                       cmap='jet', alpha=.5, zorder=5)
        ...
        sigma = model.scalery.inverse_transform(sigma.diagonal(0, -2, -1).reshape((-1, mu.shape[-1]))).reshape(
            (sigma.shape[0], -1, sigma.shape[-1]))[..., IDX][None, ...]
        mu = mu[n_xtrain:, ...]
        mu = model.scalery.inverse_transform(mu.reshape((-1, mu.shape[-1]))).reshape((mu.shape[0], -1, mu.shape[-1]))[
            ..., IDX][None, ...]
        prior = prior[None, n_xtrain:]

        Y = np.logspace(np.log10(self.data['test']['y'][:, IDX].min() * .5),
                        np.log10(self.data['test']['y'][:, IDX].max() * 1.5), 100)[::-1, None, None]
        var = 2 * sigma ** 2
