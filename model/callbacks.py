        ...
        if (self._step_count % (self.args.n_iter // self.args.n_redraws)) == 0:
            ...
            if not round_stats_file.exists() or self._step_count == 0:
                with round_stats_file.open('w+') as fn:
                    fn.write(','.join(['iteration', 'cumulative_time'] + [f'{k}_{m.__name__}' for k in all_keys for m in
                                                                          self.metrics]) + '\n')

            stats = [[str(m(y1, y2)) for y1, y2 in zip(self.data[k]['y'].T, estimates[k].T)] for k in all_keys for m in
                     self.metrics]
