...
def get_labels(wavelengths, slices, n_out=None, wavelengths_ad_ag=None, wavelengths_aph=None, use_HICO_aph=False):
    ...
    return [k + f'{wavelengths_ad_ag[i]:.0f}' if (v.stop - v.start) > 1 and (
            k == 'ad' or k == 'ag') and use_HICO_aph else k + f'{wavelengths_aph[i]:.0f}' if (
                                                                                                     v.stop - v.start) > 1 and (
                                                                                                     k == 'aph') and use_HICO_aph else k + f'{wavelengths[i]:.0f}' if (
                                                                                                                                                                              v.stop - v.start) > 1 else k + ''
            for k, v in sorted(slices.items(), key=lambda s: s[1].start)
            for i in range(v.stop - v.start)][:n_out]
...
def generate_config(args, create=True, verbose=True):
    ...
    if args.sensor in ['PRISMA', 'HICO', 'PACE'] and args.product == 'aph,chl,tss,pc,ad,ag,cdom': dependents += [
        'allow_missing', 'allow_nan_inp', 'allow_nan_out', 'filter_ad_ag', 'min_in_out_val', 'removed_dataset']
...
def _load_datasets(keys, locs, wavelengths, allow_missing=False, filter_ad_ag_bool=True, args=None):
    ...
    def loadtxt(name, loc, required_wvl, args=None):
        ...
        if 'aph' in name or 'ag' in name or 'ad' in name:
            if args.use_HICO_aph:
                if 'HICO' in args.sensor:   required_wvl = get_sensor_bands('HICO-aph', args)
                if 'PRISMA' in args.sensor: required_wvl = get_sensor_bands('PRISMA-aph', args)
                if 'PACE' in args.sensor:    required_wvl = get_sensor_bands('PACE-aph', args)
                if 'OLCI' in args.sensor:    required_wvl = get_sensor_bands('OLCI-aph', args)

        if 'ag' in name or 'ad' in name or 'bbp_Gordon' in name:
            if args.use_HICO_aph:
                if 'HICO' in args.sensor:    required_wvl = get_sensor_bands('HICO-adag', args)
                if 'PRISMA' in args.sensor:  required_wvl = get_sensor_bands('PRISMA-adag', args)
                if 'PACE' in args.sensor:    required_wvl = get_sensor_bands('PACE-adag', args)
                if 'OLCI' in args.sensor:    required_wvl = get_sensor_bands('OLCI-adag', args)

        if '../MULT/bbp' == name:
            if 'HICO' in args.sensor:    required_wvl = get_sensor_bands('bbp', args)
            if 'PRISMA' in args.sensor:  required_wvl = get_sensor_bands('bbp', args)
        ...
        try:
            ...
            data = np.loadtxt(dloc, delimiter=',',
                              dtype=float if name not in ['../Dataset', '../dataset', '../meta', '../datetime',
                                                          '../folder_name', '../latlon'] else str, comments=None)
            ...
            if data.shape[1] > 1 and data.dtype.type is not np.str_:
                ...
                if allow_missing:
                    ...
                    data = data[:, get_valid(dloc.stem, loc, required_wvl, margin=4,
                                             get_available=True if 'MULT' in name else False)]
    ...
    for loc in locs:
        try:
            ...
            if '../dataset' in keys: l_data += list(zip(list(
                ['SeaBASS2' if 'SeaBASS2' in str(i.item()) else 'SeaBASS' if '""' in str(i.item()) else str(i.item())
                 for i in loc_data[keys.index('../dataset')]]), np.arange(len(loc_data[keys.index('../dataset')]))))
            ...
            if '../dataset' not in keys: l_data += list(
                zip([loc.parent.name] * len(x_data[-1]), np.arange(len(x_data[-1]))))
    ...
    slices = []
    key_start = 2
    for i, key in enumerate(keys[key_start:]):
    ...
    keys = [k for i, k in enumerate(keys[key_start:]) if i not in drop]
    for y in y_data:
...
def get_data(args):
    ...
    x_data, y_data, slices, sources = _load_datasets(data_keys, sensor_loc, bands,
                                                     ...
                                                     filter_ad_ag_bool=args.filter_ad_ag, args=args)
    ...
    if '-nan' not in args.sensor:
        (x_data, *_), (y_data, *_), (sources, *_) = _filter_invalid(x_data, y_data, slices, other=[sources],
                                                                    allow_nan_out=not using_feature(args,
                                                                                                    'sim') and len(
                                                                        data_keys) > 2,
                                                                    min_in_out_val=args.min_in_out_val,
                                                                    datasets_to_remove=args.removed_dataset)
...
def calc_spectral_slope(wavelengths, sample, reference_wavelength=443, min_wavelength=430, max_wavelength=650,
                        slope_min_bound=0.005, slope_max_bound=0.03, allowed_error=10, ftol=1e-4, xtol=1e-3):
    from scipy.optimize import curve_fit
    ...
    if len(sample):
        if np.all(np.isfinite(sample)) and np.min(sample) > -0.1:
            try:
                ...
                params, _ = curve_fit(exponential, x, sample, p0=(1, 0.018), bounds=((1e-3, slope_min_bound), (
                    6e1, slope_max_bound)))  # (1,0.01,0),bounds=((1e-3, 4e-3, 0), (1e2, 3e-1, 1e-1)))
                new_sample = exponential(x, *params)
...
def convert_spectral_cdom_to_point_slope(wavelengths, spectral_CDOM, reference_CDOM_wavelength=443,
                                         spectral_min_max=[430, 640], allowed_error=100, ftol=1e-4, xtol=1e-3):
    # Find all wavelengths in the spectral min/max region
    wavelengths = list(wavelengths)
    spectral_CDOM = list(spectral_CDOM)
    bounded_wavelengths = [wavelength for wavelength in wavelengths if
                           wavelength < max(spectral_min_max) and wavelength > min(spectral_min_max)]
    bounded_spectral_CDOM = [spectral_CDOM[wavelengths.index(bounded_wavelength)] for bounded_wavelength in
                             bounded_wavelengths]
    ...
    S_CDOM_max = 0.030
    wavelengths_OG, sample_OG, wavelengths_new, new_sample, params, sample_resized = calc_spectral_slope(
        wavelengths=bounded_wavelengths, sample=bounded_spectral_CDOM, reference_wavelength=reference_CDOM_wavelength,
        min_wavelength=min(spectral_min_max), max_wavelength=max(spectral_min_max), slope_min_bound=S_CDOM_min,
        slope_max_bound=S_CDOM_max, allowed_error=allowed_error, ftol=ftol, xtol=xtol)
    CDOM = params[0]
    SCDOM = params[1]
