...
class RatioTransformer(_CustomTransformer):
    ...
    def __init__(self, wavelengths, *args, excl_Rrs=False, all_ratio=False, **kwargs):
        assert (not all_ratio or len(
            wavelengths) < 16), 'Too many features would be created with > 15 bands (i.e. > 1000 features)'
    ...
    def _transform(self, X, *args, **kwargs):
        ...
        assert (len(x_new) == len(
            self.labels)), f'Mismatch between features and labels: {len(x_new)} vs {len(self.labels)}'
        if self.excl_Rrs:
