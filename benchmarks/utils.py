...
def get_required(Rrs, waves, required=[], tol=5):
    ...
    return lambda w, validate=True: Rrs[
        ..., find_wavelength(w, waves, tol=tol, validate=validate)] if w is not None else Rrs
...
class Optimizer:
    ...
    def fit(self, X, Y, wavelengths):
        ...
        res = gbrt_minimize(cost_func, init, n_random_starts=10000,
                            n_calls=10000)  # , method='SLSQP')#, tol=1e-10, options={'maxiter':1e5}, method='SLSQP')
