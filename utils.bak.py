...
def _load_datasets(keys, locs, wavelengths, allow_missing=False,filter_ad_ag_bool=True,args=None):
    ...
    for loc in locs:
        try:
            ...
            if '../dataset' not in keys: l_data  += list(zip([loc.parent.name] * len(x_data[-1]), np.arange(len(x_data[-1]))))
...
def get_data(args):
    ...
    x_data, y_data, slices, sources = _load_datasets(data_keys, sensor_loc, bands, allow_missing=('-nan' in args.sensor) or (getattr(args, 'align', None) is not None),filter_ad_ag_bool=args.filter_ad_ag,args=args)
    ...
    if '-nan' not in args.sensor:
        (x_data, *_), (y_data, *_), (sources, *_) = _filter_invalid(x_data, y_data, slices, other=[sources], allow_nan_out=not using_feature(args, 'sim') and len(data_keys) > 2, min_in_out_val=args.min_in_out_val, datasets_to_remove=args.removed_dataset)
